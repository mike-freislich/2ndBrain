...
- **Strict warnings enabled:** `UNTYPED_DECLARATION`, `INFERRED_DECLARATION`, `UNSAFE_PROPERTY_ACCESS`, `UNSAFE_METHOD_ACCESS`, `UNSAFE_CAST`, `UNSAFE_CALL_ARGUMENT`, `RETURN_VALUE_DISCARDED`, `INTEGER_DIVISION` — all set to *Error*.
...
- **One responsibility per scene.** If a scene's script is >300 lines or its node tree has multiple unrelated subsystems, split it.
